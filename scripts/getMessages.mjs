...
if (!threadId) {
  console.error("Please provide a thread id with `node getMessages.js <id>`");
  process.exit(1);
}
let file;
try {
  file = fs.readFileSync("threads.json", "utf8");
} catch (err) {
  // if file doesnt exist throw error
  if (err.code === "ENOENT") {
    console.error(
      "threads.json not found. Please run `node getThreads.js threads.json` to generate threads.json"
    );
  } else {
    console.error(err);
  }
...
async function getCursor(cid, dtsg) {
  ...
  const { cursor } = await parseResponse(
...
ws.on("close", function close() {
  ...
  fs.writeFile(args[1], JSON.stringify(messages, null, 2), (err) => {
    if (err) {
      console.error(err);
    }
