...
function parse_sign_and_base(s) {
  ...
  if (s.codePointAt(i) === /* '0' */48) {
    const match$1 = s.codePointAt(i + 1 | 0)
    if (match$1 >= 89) {
      if (match$1 >= 111) {
        if (match$1 < 121) {
...
function int_of_string(s) {
  ...
  const len = s.length
  const c = i < len ? s.codePointAt(i) : /* '\000' */0
  const d = parse_digit(c)
  if (d < 0 || d >= base) {
    throw new Error('Failure: int_of_string')
  }
  const aux = (_acc, _k) => {
...
function int64_of_string(s) {
  ...
  if (Caml.i64_lt(d, Caml_int64.zero) || Caml.i64_ge(d, base)) {
    throw new Error('Failure: int64_of_string')
  }
...
function parse_format(fmt) {
  ...
  if (len > 31) {
    throw new Error('Invalid_argument: format_int: format too long')
  }
  const f = {
    justify: '+',
    signstyle: '-',
      ...
      switch (c) {
        ...
        case 46:
          ...
          while ((function (j) {
            // eslint-disable-next-line func-names
            return function () {
              const w = fmt.codePointAt(j) - /* '0' */48 | 0
              return w >= 0 && w <= 9
            }
          }(j))()) {
    ...
    switch (exit) {
      ...
      case 3:
        ...
        while ((function (j$1) {
          // eslint-disable-next-line func-names
          return function () {
            const w = fmt.codePointAt(j$1) - /* '0' */48 | 0
            return w >= 0 && w <= 9
          }
        }(j$1))()) {
      ...
      case 'g':
        ...
        if (exp < -4 || x$1 >= 1e21 || x$1.toFixed().length > prec$1) {
          ...
          if (exp < 0) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-loop-func, func-names
            while ((function () {
              s = x$1.toFixed(p)
...
// eslint-disable-next-line func-names
const hexstring_of_float = (function (x, prec, style) {
  if (!Number.isFinite(x)) {
    if (Number.isNaN(x)) return 'nan'
    return x > 0 ? 'infinity' : '-infinity'
  }
  // eslint-disable-next-line eqeqeq
  const sign = (x == 0 && 1 / x == -Infinity) ? 1 : (x >= 0) ? 0 : 1
  // eslint-disable-next-line no-param-reassign
  if (sign) x = -x
  let exp = 0
...
const float_of_string = ((s, exn) => {
  ...
  if (m) {
    ...
    const mantissa = parseInt(m[1] + m[2] + m3, 16)
    const exponent = (m[4] | 0) - 4 * m3.length
