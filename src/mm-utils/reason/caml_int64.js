...
export function float_of_bits(x) {
  return ((lo, hi) => (new Float64Array(new Int32Array([lo, hi]).buffer))[0])(x[1], x[0])
}

export function bits_of_float(x) {
  const match = (_x => new Int32Array(new Float64Array([_x]).buffer))(x)
  return [
    match[1],
    (match[0] >>> 0),
  ]
