...
export default class InstagramWebSocket {
  ...
  fetchMoreInboxThreads = (filter: ThreadFilter) => {
    ...
    return this.publishTask('fetch more inbox threads', [
      {
        label: '313',
        payload: JSON.stringify({
          cursor: this.papi.kv.get('cursor-1'),
  ...
  fetchInboxThreads = () => {
    ...
    const group95Primary = this.papi.api.getSyncGroupThreadsRange(SyncGroup.UNKNOWN, ParentThreadKey.PRIMARY)
    this.logger.info('fetchInboxThreads', {
      group1General,
      group1Primary,
      group95General,
      group95Primary,
