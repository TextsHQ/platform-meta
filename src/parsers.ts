...
import type { ExtendedIGMessage, ExtendedIGThread, IGAttachment, IGReaction } from './ig-types'
import type { IGThread, IGMessage, IGParticipant } from './store/schema'
...
export function parsePayload(myUserId: string, payload: string) {
  ...
  for (const item of lsCalls.deleteThenInsertThread) {
    ...
    conversations.push({
      ...
      lastMessageCtaTimestampMs: getAsDate(item[67]),
